...
*/
	...
	if (ROTATE_STEPS > 1) {
	  ...
	  sprintf(outfile, "%s_%ld.tiff",filename, 1000+step);
        sprintf(outfile2, "%s_%ld.txt", "saida", 1000 + step);
#endif
/*	  Store_Image(outfile);
          p = image_address;
...
tiff_save_rgba(filename,tiff_image,image_len[X],image_len[Y]);    */
	  ...
	  WriteGrayscaleTIFF(filename, outfile2, image_len[X],image_len[Y],image_len[X], image_address);
	}
      }
    }
#ifdef DIM
...
long WriteGrayscaleTIFF(char *filename, char *filename_txt,  long width, long height, long scanbytes, unsigned char *data)
{
  ...
  int id = 0;
    ...
    //sprintf(nome_arquivo, "%s%d%s", "teste", id, ".txt");
